<?php 
$num  = file_get_contents(dirname(__DIR__, 2).'/common/inc_new/num_koe.inc');
$qnum = file_get_contents(dirname(__DIR__, 2).'/common/inc_new/num_enquete.inc');
...
$crumbs = array(array('@type' => 'ListItem', 'position' => 1, 'name' => 'トップ', 'item' => 'https://test.dm110.jp/'));
$parts = array_filter(explode('/', $req_uri));
$pos = 2;
$cb = 'https://test.dm110.jp';
foreach($parts as $p) {
    if(empty($p) || $p == 'test' || $p == 'index.php') continue;
    $cb .= '/' . $p;
    $crumbs[] = array(
        '@type' => 'ListItem',
        'position' => $pos++,
        'name' => htmlspecialchars($p),
        'item' => $cb . '/'
    );
}
if(count($crumbs) > 1):
...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": <?= json_encode($crumbs, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>
}
</script>
<?php endif; ?>
